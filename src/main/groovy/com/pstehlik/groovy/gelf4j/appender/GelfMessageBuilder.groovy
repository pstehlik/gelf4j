...
class GelfMessageBuilder {
  ...
  GelfMessageBuilder(Gelf4JAppender appender, LoggingEvent loggingEvent) {
    ...
    if (appender.includeLocationInformation && loggingEvent.locationInformationExists()) {
      setAdditionalField('code_location_file', loggingEvent.locationInformation.fileName)
      setAdditionalField('code_location_line', loggingEvent.locationInformation.lineNumber)
      setAdditionalField('code_location_method_name', loggingEvent.locationInformation.methodName)
    }

    setAdditionalField('logger_name', loggingEvent.loggerName)
